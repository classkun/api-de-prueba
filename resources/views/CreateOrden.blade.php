...
<script>
    ...
    $(document).ready(function () {
        ...
        var table = $('#table').DataTable({
            ...
            columns: [
                ...
                {"render": function (data, type, row) {
                        ...
                        return '<input type="number" id="number_'+row.index+'"></input>';
                    },},
                {
        ...
        updateTableJson = function(){
            ...
            let elemento = {'id':select.value,'descripcion':select.options[select.selectedIndex].text,'index' : index,'costo':productData[select.selectedIndex].costo};
    ...
    function PDF(dataAjax){
        let url = '../../Orden/PDF?total='+dataAjax.total+'&impuesto='+dataAjax.impuesto+'&comentario='+dataAjax.comentario+'$id='+dataAjax.id;
        let cantidades = dataAjax.cantidades;
        let productos = dataAjax.nproductos;
        for(let i = 0 ; i< productos.length; i++){
            url +='&nproductos%5B%5D='+productos[i];

        }
        for(let i = 0 ; i< cantidades.length; i++){
            url +='&cantidades%5B%5D='+cantidades[i];
    ...
    function SendEmail(idOrden){
        
        dataAjax = {'name' : "usuario", 'address' : $('#inputEmail').val(),'id':idOrden};
        $.ajax({
        type: "Post",
        url: '../../Orden/sendEmail',
        headers: {
    ...
    function Registrar(){
        ...
        for(let i = 0 ; i < jsonProductos.length ; i++){
            jsonProductos[i].cantidad = parseInt($("#number_"+i).val()); 
            cantidades[i] = jsonProductos[i].cantidad;
            idproductos[i] = jsonProductos[i].id;
            nombreProductos[i] = jsonProductos[i].descripcion;
            total += parseInt(jsonProductos[i].cantidad)*parseInt(jsonProductos[i].costo);
        }
        let dataAjax = {
            "total": total*1.16,
            "impuesto": total*0.16,
